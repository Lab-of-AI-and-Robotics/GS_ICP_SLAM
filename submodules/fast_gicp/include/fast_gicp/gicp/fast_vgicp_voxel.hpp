...
public:
  ...
  std::pair<meanvec, covvec>  getVoxelMeanCov(){
    std::vector<Eigen::Vector4d, Eigen::aligned_allocator<Eigen::Vector4d>> means;
    std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>> covs;
    for (auto& voxel:voxels_ ){
      means.push_back(voxel.second->mean);
      covs.push_back(voxel.second->cov);
    }
    return std::make_pair(means, covs);
  }
