...
namespace sibr
{
	...
	std::vector<InputCamera::Ptr> InputCamera::DummyCam(const std::string& transformPath, int w, int h, std::string extension, const float zNear, const float zFar, const int offset, const int fovXfovYFlag)
	{
		// std::ifstream json_file(transformPath, std::ios::in);
		...
		if (fovXfovYFlag) {
			// camera = std::make_shared<InputCamera>(InputCamera(focaly, focalx, 0.0f, 0.0f, int(w), int(h), i + offset));
			camera = std::make_shared<InputCamera>(InputCamera(focaly, focalx, 0.0f, 0.0f, int(w), int(h), offset));
		}
		else {
			// camera = std::make_shared<InputCamera>(InputCamera(focalx, 0.0f, 0.0f, int(w), int(h), i + offset));
			camera = std::make_shared<InputCamera>(InputCamera(focalx, 0.0f, 0.0f, int(w), int(h), offset));
		}
